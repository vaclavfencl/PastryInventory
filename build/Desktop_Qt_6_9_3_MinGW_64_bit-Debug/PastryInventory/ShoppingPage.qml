...
Page {
    ...
    // 0 = Buy now, 1 = Buy on Sale
    property int activeFilter: 0

    // ✅ active household for API calls (set from dropdown, initial load picks first household)
    ...
    // filtered list model displayed in repeater
    ...
    function applyFilter() {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            shoppingModel.append({
                id: String(it.id || ""),
                name: String(it.name || ""),
                categoryId: it.categoryId,            // may be null
    ...
    Component.onCompleted: {
        // initial household: pick first one
